...
namespace OgeSharp
{
    public partial class Oge
    {
        ...
        public GradeEntry GetGrades(Semester semester)
        {
            ...
            var request = Browser.CreateRequest(GradesUri);
            request.Method = "POST";
            ...
            var source = Browser.ProcessRequest(request).GetContent();
            ...
            var semesterName = document.DocumentNode.SelectSingleNode("//li[contains(@class, 'ui-state-active')]")
                .InnerText;
            ...
            foreach (var ueNode in document.DocumentNode.SelectNodes("//div[@class='moy_UE']"))
            {
                // Get the UE name
                var ueText = ueNode.SelectSingleNode("./div[1]/span").InnerText;
                var ueName = ueText.Split('(')[0].TrimEnd();
                ...
                var match = CoefficientRegex.Match(ueText);
                if (match.Success)
                {
                    ueCoefficient = double.Parse(match.Groups[1].Value, CultureInfo.InvariantCulture);
                }
                ...
                foreach (var poleNode in ueNode.SelectNodes("./div[contains(@class, 'moy')]"))
                {
                    ProcessPole(poleNode, ueEntry);
                }
            }
        ...
        private static void ProcessPole(HtmlNode poleNode, GradeEntry rootEntry)
        {
            ...
            var poleText = poleNode.SelectSingleNode(".//thead/tr/th[1]/span").InnerText;
            var poleName = poleText.Split('(')[0].TrimEnd();

            // Initialize the coefficient
            double poleCoefficient;

            // Try to get the coefficient out of the text and save it if it is valid
            var match = CoefficientRegex.Match(poleText);
            if (match.Success)
            {
                poleCoefficient = double.Parse(match.Groups[1].Value, CultureInfo.InvariantCulture);
            }
            ...
            foreach (var gradeCategoryNode in poleNode.SelectNodes(".//tbody/tr"))
            {
                ProcessGradeCategory(gradeCategoryNode, poleEntry);
            }
        }
        ...
        private static void ProcessGradeCategory(HtmlNode gradeCategoryNode, GradeEntry poleEntry)
        {
            ...
            if (gradeCategoryNode.HasClass("ui-datatable-empty-message"))
            {
                return;
            }
            ...
            var categoryName = gradeCategoryNode.SelectSingleNode("./td[1]").InnerText[0..^1];

            // If the category coefficient is not set, let's assume it's 1
            if (!double.TryParse(gradeCategoryNode.SelectSingleNode("./td[2]").InnerText, NumberStyles.Any,
                    CultureInfo.InvariantCulture, out var categoryCoefficient))
            {
            ...
            var gradesText = gradeCategoryNode.SelectSingleNode("./td[3]").InnerText;
            ...
            foreach (var line in gradesText.Split('\n',
                         StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                // Get the grade name
                var gradeName = splittedLine[0].TrimEnd();
                ...
                var matches = NumbersRegex.Matches(splittedLine[1]);

                // Initialize the grade entry
                GradeEntry gradeEntry = new(
                    gradeName,
                    double.Parse(matches[^1].Groups[1].Value, CultureInfo.InvariantCulture));
                categoryEntry.Entries.Add(gradeEntry);

                // Loop through the grades
                for (var n = 0; n < matches.Count - 1; n += 3)
                {
                    // Get the grade, max grade and coefficient
                    var grade = double.Parse(matches[n].Groups[1].Value, CultureInfo.InvariantCulture);
                    var maxGrade = double.Parse(matches[n + 1].Groups[1].Value, CultureInfo.InvariantCulture);
                    var gradeCoefficient =
                        double.Parse(matches[n + 2].Groups[1].Value, CultureInfo.InvariantCulture);
